# The core logic looping through the data
    ...
    def generate_report(payload: EvaluateRequest) -> dict[str, ModelMetrics]:
        ...
        for model_name, run in payload.runs.items():
            ...
            for query_id, ranked_docs in run.items():
                relevant = payload.qrels.get(query_id, {})
                rel_set = {doc_id for doc_id, rel in relevant.items() if rel > 0}
                
                if not rel_set:
                    continue

                ranked = ranked_docs[:payload.k]
                ...
                precision_values.append(sum(binary_relevances) / payload.k)
                
                ndcg_values.append(
                    ndcg_at_k([relevant.get(doc_id, 0) for doc_id in ranked], payload.k)
                )

            # Build the typed metrics object for this model
            report[model_name] = ModelMetrics(
