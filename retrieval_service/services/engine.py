...
Coordinates preprocessing, index loading, Strategy+Factory scoring, and document lookup.
"""
...
class RetrievalEngine:
    ...
    def search(self, request: SearchRequest) -> SearchResponse:
        ...
        index_data = index_repo.as_scoring_payload(loaded_index)
        document_repo = DocumentRepository(dataset_name)

        query_text = request.query
        if request.use_refinement:
            query_text = self.refinement_client.refine_query(request.query)
